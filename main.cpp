...
void Test3() {
	...
	{
		Obj::ResetCounters();
		SimpleVector<int> v(MEDIUM_SIZE);
		{
	...
	}
	{
		Obj::ResetCounters();
		{
			SimpleVector<Obj> v(MEDIUM_SIZE);
			v[MEDIUM_SIZE / 2].id = ID;
			assert(Obj::num_default_constructed == MEDIUM_SIZE);
			SimpleVector<Obj> moved_from_v(std::move(v));
	...
	{
		Obj::ResetCounters();
		SimpleVector<Obj> v_medium(MEDIUM_SIZE);
		v_medium[MEDIUM_SIZE / 2].id = ID;
		SimpleVector<Obj> v_large(LARGE_SIZE);
		v_large = v_medium;
	...
	assert(Obj::GetAliveObjectCount() == 0);
	{
		...
		SimpleVector<Obj> v_small(MEDIUM_SIZE / 2);
...
void Test6() {
	...
	{
		SimpleVector<int> v(vvv);
		auto* pos = v.Emplace(v.end(), 50);
	...
	}
	{
		...
		SimpleVector<Obj> v{SIZE};
		Obj obj{1};
		SimpleVector<Obj>::iterator pos = v.Insert(v.cbegin() + 1, obj);
		assert(v.Size() == SIZE + 1);
		assert(v.Capacity() == SIZE * 2);
		assert(&*pos == &v[1]);
	...
	}
	{
		Obj::ResetCounters();
		SimpleVector<Obj> v{SIZE};
		SimpleVector<Obj>::iterator pos = v.Insert(v.cbegin() + 1, Obj{1});
		assert(v.Size() == SIZE + 1);
		assert(v.Capacity() == SIZE * 2);
		assert(&*pos == &v[1]);
		assert(v[1].id == 1);
	...
	}
	{
		...
		assert(&*pos == &v[3]);
		assert(v[3].id == ID);
			std::cout << v[2].id << v[2].name << v[3].id << v[3].name << v[4].id << v[4].name << std::endl;
